...
class Prescription(models.Model):
    ...
    _description = "Prescription (Roucheta)"
    ...
    @api.depends('main_complaint_id')
    def _compute_display_main_complaint(self):
        for rec in self:
            if rec.main_complaint_id:
                last_complaint = rec.env['main.complaint'].search([], order="id desc", limit=1)
                desc = last_complaint.description or ''
                rec.display_main_complaint = f"prescription for -  {desc}"
            else:
                last_complaint = rec.env['main.complaint'].search([], order="id desc", limit=1)
                desc = last_complaint.description or ''
                rec.display_main_complaint = f"prescription for  - {desc[:50]}" if desc else "prescription complaint"
...
#         for rec in self:
#             if rec.main_complaint_id:
#                 name = f"{rec.main_complaint_id.description} - Prescription"
#             else:
#                 name = "New Prescription"
#             result.append((rec.id, name))
#         return result
